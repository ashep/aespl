...
esp_err_t aespl_httpd_send_file(httpd_req_t *req, const char *path, const char *type, const char *encoding) {
    ...
    if (type != NULL) {
        ...
        if (err != ESP_OK) {
            fclose(f);
            ESP_LOGE(LOG_TAG, "failed to set 'Content-Type' header; path=%s; err=%d", path, err);
            return err;
        }
    }
    ...
    if (encoding != NULL) {
        ...
        if (err != ESP_OK) {
            fclose(f);
            ESP_LOGE(LOG_TAG, "failed to set 'Content-Encoding' header; path=%s; err=%d", path, err);
            return err;
        }
    }
    ...
    while (!feof(f)) {
        ...
        if (nb < 1 && !feof(f)) {
            fclose(f);
            ESP_LOGE(LOG_TAG, "failed to read data from the file; path=%s; err=%d", path, nb);
            return ESP_FAIL;
        }
        ...
        if (err != ESP_OK) {
            fclose(f);
            ESP_LOGE(LOG_TAG, "failed to send data chunk; path=%s; err=%d", path, err);
            return err;
        }
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(LOG_TAG, "failed to send final data chunk; path=%s; err=%d", path, err);
        return err;
    }

    ESP_LOGI(LOG_TAG, "%s %s %s", http_method_str(req->method), req->uri, HTTPD_200);
