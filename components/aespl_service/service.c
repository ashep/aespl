...
static esp_err_t httpd_options(httpd_req_t *req) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(AESPL_SERVICE_LOG_TAG, "unable to set Access-Control-Allow-Origin");
        return err;
    }

    err = httpd_resp_set_hdr(req, "Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    if (err != ESP_OK) {
        ESP_LOGE(AESPL_SERVICE_LOG_TAG, "unable to set Access-Control-Allow-Methods");
        return err;
    }

    err = httpd_resp_set_hdr(req, "Access-Control-Allow-Headers", "Content-Type");
    if (err != ESP_OK) {
        ESP_LOGE(AESPL_SERVICE_LOG_TAG, "unable to set Access-Control-Allow-Headers");
        return err;
    }
...
static esp_err_t httpd_post_wifi_scan(httpd_req_t *req) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(AESPL_SERVICE_LOG_TAG, "esp_wifi_scan_start() failed: %#x", err);
        return err;
    }
    ...
    if (err == ESP_OK) {
        for (uint16_t i = 0; i < n_rec; i++) {
            ...
            ESP_LOGD(AESPL_SERVICE_LOG_TAG, "found WiFi AP: %s, %ddBm", scan_res.ssid, scan_res.rssi);
        }
        aespl_httpd_send_json_result(req, HTTPD_200, res);
    } else {
        ESP_LOGE(AESPL_SERVICE_LOG_TAG, "esp_wifi_scan_get_ap_records() failed: %#x", err);
...
static esp_err_t httpd_post_wifi(httpd_req_t *req) {
    ...
    if (req_json == NULL) {
        ESP_LOGW(AESPL_SERVICE_LOG_TAG, "httpd_post_wifi(): failed to parse incoming JSON");
    ...
    if (!cJSON_IsString(ssid)) {
        ESP_LOGW(AESPL_SERVICE_LOG_TAG, "httpd_post_wifi(): 'ssid' field is not valid");
    ...
    if (!cJSON_IsString(pass)) {
        ESP_LOGW(AESPL_SERVICE_LOG_TAG, "httpd_post_wifi(): 'password' field is not valid");
        cJSON_Delete(req_json);
        aespl_httpd_send_json_error(req, HTTPD_400, ESP_ERR_INVALID_ARG);
        return ESP_ERR_INVALID_ARG;
    }

    ESP_LOGI(AESPL_SERVICE_LOG_TAG, "WiFi settings received: \"%s\", \"%s\"", ssid->valuestring, pass->valuestring);
    ...
    if (err != ESP_OK) {
        ESP_LOGW(AESPL_SERVICE_LOG_TAG, "esp_wifi_set_config(): %#x, %s", err, esp_err_to_name(err));
        aespl_httpd_send_json_error(req, HTTPD_500, err);
        return err;
    }
...
esp_err_t aespl_service_init(aespl_httpd_t *httpd) {
    ...
    err = aespl_httpd_handle(httpd, HTTP_POST, uri, httpd_post_wifi_connect, NULL);
    if (err != ESP_OK) {
        return err;
    }
    err = aespl_httpd_handle(httpd, HTTP_OPTIONS, uri, httpd_options, NULL);
    ...
    err = aespl_httpd_handle(httpd, HTTP_POST, uri, httpd_post_wifi_disconnect, NULL);
    if (err != ESP_OK) {
        return err;
    }
    err = aespl_httpd_handle(httpd, HTTP_OPTIONS, uri, httpd_options, NULL);
